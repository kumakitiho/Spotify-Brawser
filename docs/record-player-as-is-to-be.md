...
`Shelf Mode` のレコードプレーヤーUIを、添付された上面視のターンテーブル画像に寄せつつ、スマホ縦画面でも見やすい比率にするためのデザインメモです。

![Realistic Record Player UI Mock](./record-player-ui-mock.svg)

## 目的

現在の画面には、レコード棚・中央レコード・トーンアーム・プログレス表示の土台があります。次の改善では、見た目を足すだけでなく、**実機っぽいターンテーブルの構造と、スマホ縦画面に収まるレイアウト** を入れます。
...
| 左下/盤右下スイッチ | 盤面と重なって見えるため、現モックでは削除する |
...
- SVGモックはスマホ縦画面を想定し、`900 × 1200` の縦長キャンバスにする
...
- 左下の `start · stop` 系スイッチと、盤右下の小ボタンは盤面と干渉するため配置しない
...
1. **まず見た目**: 縦長筐体、巨大プラッター、固定ストロボドット、S字アーム、ピッチスライダー
